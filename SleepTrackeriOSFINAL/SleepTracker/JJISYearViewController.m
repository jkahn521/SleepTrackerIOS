...
#import "JJISYearViewController.h"
#import "CorePlot-CocoaTouch.h"
#import "JJISAppDelegate.h"

@interface JJISYearViewController ()

@property (nonatomic, strong) IBOutlet CPTGraphHostingView *hostView;
@property (nonatomic, strong) CPTBarPlot *Month;
@property (nonatomic, strong) CPTPlotSpaceAnnotation *dayAnnotation;
@property (nonatomic, strong) NSMutableArray* last12Months;
...
@implementation JJISYearViewController

CGFloat const CPDBarWidth12 = 0.35f;
CGFloat const CPDBarInitialX12 = .025f;
...
// configureHost configures the CorePlot rectangle
-(void) configureHost{
...
-(void) initPlot{
    ...
    self.last12Months = [appDelegate.manager getLastYearInfo:self.today];
    
    self.xLabels = [appDelegate.manager getMonthNames:self.today];
    self.max = [appDelegate.manager getDataMax];
    self.yearMax = [appDelegate.manager getYearMax];
    [self configureHost];
    [self configureGraph];
    [self configurePlots];
    [self configureAxes];
...
-(void) configureGraph{
    ...
    CGFloat xMax = 12.0f;
    CGFloat yMin = 0.0f;
    CGFloat yMax = self.yearMax + 5.0f;  // should determine dynamically based on max price
...
-(double)doubleForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)idx{
    if(fieldEnum == 0){
        ...
        if ([plot.identifier isEqual:self.Month.identifier]){
            NSNumber* duration = [self.last12Months objectAtIndex:idx];
...
-(void) configurePlots{
    self.Month = [CPTBarPlot tubularBarPlotWithColor:[CPTColor blueColor] horizontalBars:NO];
    self.Month.identifier = @"Month";
    
    
    // 2 - Set up line style
    CPTMutableLineStyle *barLineStyle = [[CPTMutableLineStyle alloc] init];
    barLineStyle.lineColor = [CPTColor blueColor];
    barLineStyle.lineWidth = 0.35;
    // 3 - Add plots to graph
    CPTGraph *graph = self.hostView.hostedGraph;
    CGFloat barX = CPDBarInitialX12;
    
    NSArray *plots = [NSArray arrayWithObjects:self.Month, nil];
    for (CPTBarPlot *plot in plots) {
        plot.dataSource = self;
        plot.delegate = self;
        plot.barWidth = CPTDecimalFromDouble(CPDBarWidth12);
...
-(NSUInteger)numberOfRecordsForPlot:(CPTPlot *)plot{
    return self.last12Months.count;
}
