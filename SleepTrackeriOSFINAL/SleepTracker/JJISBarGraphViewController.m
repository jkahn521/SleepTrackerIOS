...
-(void) initPlot{
    ...
    self.lastWeek = [appDelegate.manager getLastNumDayTotals:self.today :7];
    
    self.xLabels = [appDelegate.manager getDayNames:self.today:true];
...
-(void) configureGraph{
    ...
    // 4 - Set up title
    NSString *title = @"Last Week";
    graph.title = title;
    graph.titleTextStyle = titleStyle;
    graph.titlePlotAreaFrameAnchor = CPTRectAnchorTop;
    graph.titleDisplacement = CGPointMake(-42.0f, -16.0f);
    // 5 - Set up plot space
    CGFloat xMin = 0.0f;
    CGFloat xMax = 7.0f;
    CGFloat yMin = 0.0f;
...
-(NSString *) legendTitleForBarPlot:(CPTBarPlot *)barPlot recordIndex:(NSUInteger)idx{
  //  NSLog(@"%lu", (unsigned long)idx);
    if([barPlot.identifier isEqual:self.Nap.identifier] && idx == 1){
        return @"Nap";
    }
    if( idx == 0){
        return @"Sleep";
...
-(double)doubleForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)idx{
    if(fieldEnum == 0){
        ...
        if ([plot.identifier isEqual:self.Day.identifier]){
            ...
            NSNumber* duration2 = [self.lastWeekNaps objectAtIndex:idx];
            NSNumber* z = [NSNumber numberWithFloat:[duration floatValue] - [duration2 floatValue]];
            return [z floatValue];
        }
...
-(void) configurePlots{
    self.Day = [CPTBarPlot tubularBarPlotWithColor:[CPTColor grayColor] horizontalBars:NO];
    self.Day.identifier = @"Day";
    
    self.Nap = [CPTBarPlot tubularBarPlotWithColor:[CPTColor blueColor] horizontalBars:NO];
    ...
    NSArray *plots = [NSArray arrayWithObjects:self.Nap, self.Day, nil];
